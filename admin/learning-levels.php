...
<main class="p-4 sm:ml-64">
  <div class="p-4 rounded-lg mt-14">
    <section class="flex flex-col md:flex-row gap-4 justify-between my-8">
      <h1 class="text-2xl font-bold">Learning Levels</h1>
      <button onclick="showModal()" class="py-2.5 px-5 bg-blue-700 hover:bg-blue-800 rounded-lg text-white max-w-xs w-full">New Level</button>
    </section>
    <section class="flex">
      <table id="datatable" class="ui sixteen wide centered column table" width="100%">
        ...
        <tbody>
          ...
          <?php foreach($learning_levels as $item): extract($item) ?>
          <tr onclick="viewItem(<?php echo $level_id ?>)" class="cursor-pointer">
  ...
  <div class="relative p-4 w-full m-auto flex flex-col justify-center max-w-2xl h-screen">
    ...
    <div class="relative bg-white rounded-lg shadow">
      ...
      <div class="p-4 md:p-5 space-y-4">
        <form action="./action.php?action=insert_learning_level" method="POST" enctype="multipart/form-data" class="space-y-4">
          <input type="text" name="level_id" id="level_id" class="hidden">
          <div class="grid grid-cols-1 gap-2">
            <div class="grid gap-1.5">
              <label for="">Title</label>
              <input type="text" name="level_name" id="level_name" class="py-2 px-3 border w-full rounded-lg">
            </div>
            <div class="grid gap-1.5">
...
<script>
  ...
  function viewItem(level_id) {
    $.get(`./action.php?action=get_learning_level&level_id=${level_id}`, (res) => {
