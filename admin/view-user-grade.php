...
<?php $user = $crud->getUser($_GET["user_id"], false); ?>
<?php $user_classrooms = $crud->getUserClassrooms($_GET["user_id"]);
extract($user_classrooms); ?>

<main class="p-4 sm:ml-64">
  <div class="p-4 rounded-lg mt-14">
    <div class="flex justify-center">
      <button id="printGrade" class="flex gap-2 py-2.5 px-5 bg-blue-700 hover:bg-blue-800 text-white rounded-lg">
        <i data-lucide="printer"></i>
        Download Report
      </button>
    </div>
    <div id="printPDF" class="grid gap-4 md:grid-cols-1 place-items-center p-6 md:p-10">
        ...
        <table class="ui sixteen wide centered column celled table table-fixed" width="100%">
          <thead>
            <tr>
              ...
              <th>Examinee No. <?php echo $_GET["user_id"] ?> </th>
              <th>Examinee: <br> <?php echo $user["fullname"] ?></th>
            </tr>
          </thead>
        </table>
        <table class="ui sixteen wide centered column celled table table-fixed" width="100%">
          ...
          <tbody>
            <?php $user_grades = $crud->getUserGrades($_GET["user_id"]); ?>
